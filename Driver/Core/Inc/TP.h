...
void showzifu(unsigned int x, unsigned int y, unsigned char value, unsigned int dcolor, unsigned int bgcolor)
{
	...
	LCD_SetPos_Vertical(x, x + 7, y, y + 11);
	temp += (value - 32) * 12;
	for (j = 0; j < 12; j++)
	{
		for (i = 0; i < 8; i++)
		{
			if ((*temp & (1 << (7 - i))) != 0)
			{
				Write_Data_int(dcolor);
			}
			else
			{
				Write_Data_int(bgcolor);
			}
		}
		temp++;
	}
...
void TPTEST(void)
{
	...
	Write_Data_int(0x00);
	LCD_SetPos_Vertical(lx, lx + 1, ly, ly + 1);
	for (k = 0; k < 2; k++)
	{
		for (h = 0; h < 2; h++)
		{

			Write_Data_int(ER_TFT040_CONVERT_DATA(0xf1, 0x00));
		}
	}
}
